...
create table if not exists lecture (created_at datetime(6), instructor_id bigint, lecture_id bigint not null auto_increment, seat_count bigint, start_time datetime(6), updated_at datetime(6), venue_id bigint, description varchar(255), title varchar(255), status enum ('CANCELED','REGISTER'), primary key (lecture_id)) engine=InnoDB;
